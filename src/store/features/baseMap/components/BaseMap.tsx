...
const BaseMap: FC<{
	...
	return layersReverse.map((layer: LayerConfig) => {
				...
				case 'online-raster-xyz':
					...
					return (
						<LayerBitmapTile
							...
							enabledZoomMin={
								(layer.options as LayerConfigOptionsOnlineRasterXYZ).enabledZoomMin
							}
							enabledZoomMax={
								(layer.options as LayerConfigOptionsOnlineRasterXYZ).enabledZoomMax
							}
							url={get(layer.options, 'url', '')}
							alpha={(layer.options as LayerConfigOptionsOnlineRasterXYZ).alpha}
							cacheSize={
								(layer.options as LayerConfigOptionsOnlineRasterXYZ).cacheSize
							}
							cacheDirChild={stringifyProp(
								(layer.options as LayerConfigOptionsOnlineRasterXYZ).url || ''
							)}
							cacheDirBase={
								(cacheDirBase ?? '/') as LayerBitmapTileProps['cacheDirBase']
				...
				case 'raster-MBtiles':
					return (
						<LayerMBTilesBitmap
							...
							enabledZoomMin={
								(layer.options as LayerConfigOptionsRasterMBtiles).enabledZoomMin
							}
							enabledZoomMax={
								(layer.options as LayerConfigOptionsRasterMBtiles).enabledZoomMax
							}
				...
				case 'mapsforge':
					if (profiles.length > 0) {
						let profile = profiles.find(
							(prof) =>
								prof.key === (layer.options as LayerConfigOptionsMapsforge).profile
						);
						...
						return (
							<LayerMapsforge
								key={layer.key}
								enabledZoomMin={
									(layer.options as LayerConfigOptionsMapsforge).enabledZoomMin
								}
								enabledZoomMax={
									(layer.options as LayerConfigOptionsMapsforge).enabledZoomMax
								}
				...
				case 'hillshading':
					...
					return (
						<LayerHillshading
							...
							enabledZoomMin={
								(layer.options as LayerConfigOptionsHillshading).enabledZoomMin
							}
							enabledZoomMax={
								(layer.options as LayerConfigOptionsHillshading).enabledZoomMax
							}
							...
							cacheDirBase={
								(cacheDirBase ?? '/') as LayerHillshadingProps['cacheDirBase']
							} // if ``, will fallback to cache dbname;
							shadingAlgorithm={
								(layer.options as LayerConfigOptionsHillshading).shadingAlgorithm
							}
