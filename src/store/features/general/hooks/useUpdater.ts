...
const useUpdater = ( {
    ...
    const runUpdates = useCallback( ( updateCbsKeys: string[] ): Promise<UpdateResults> => new Promise( ( resolve, reject ) => {
        updateCbsKeys.sort( semverCompare );
        const results: UpdateResults = {};
        resolve( [...updateCbsKeys].reduce( ( accumulatorPromise: Promise<UpdateResults>, versionFrom: string, idx ) => {
            const cb = ( results: UpdateResults ): Promise<UpdateResults> => new Promise( resolveCb => {
                const updateCb = get( updateCbs, versionFrom );
				const versionFromNext = get( updateCbsKeys, idx + 1, packageJson.version );
                setIsUpdating( {
                    ...results,
    ...
    useEffect( () => {
        if ( ready ) {
            ...
            } else if ( true === isUpdating ) {	// the initial value. That should only run on init when ready. Thats why the following is wrapped in a timeout.
				setTimeout( () => {
