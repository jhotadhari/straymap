...
const strValToNb = (val: string, numType: NumType) => {
	switch (true) {
		case numType === 'int':
			return parseInt(
...
export const NumericRowControl = ({
	...
	const onBlur = () => {
		let newValNb = strValToNb(val, numType);
		if ('number' !== typeof newValNb || isNaN(newValNb) || (validate && !validate(newValNb))) {
			// reset val
	...
	useEffect(() => {
		if (!blurred && !keyboardShown && val !== initialVal) {
			onBlur();
		}
	}, [
		initialVal,
...
export const NumericMultiRowControl = ({
	...
	// Blur on keyboard hide.
	useEffect(() => {
		if (!blurred && !keyboardShown && vals.toString() !== initialVals.toString()) {
			[...optKeys].map((optKey, index) => {
				onBlur(index, optKey);
			});
		}
