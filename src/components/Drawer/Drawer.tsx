...
const DrawerInner = ({
	...
	const DisplayComponent = get(drawerElementComponents, [
		activeElement.type as string,
		'DisplayComponent',
	]);
	if (!DisplayComponent) {
		return null;
...
const DrawerHandle = ({
	...
	const iconSource = IconComponent
		? undefined
		: get(drawerElementComponents, [element.type as string, 'iconSource']);
	const color =
		element.type === activeElementKey
		...
		>
			<View
				style={{
					...
					backgroundColor:
						element.type === activeElementKey ? theme.colors.background : 'transparent',
			...
			>
				...
				{element.type === activeElementKey && IconActions && (
					<View
						style={{
							position: 'absolute',
...
const Drawer = ({
	...
	const theme = useTheme();
	const [activeElementKey, setActiveElementKey] = useState<string | undefined>(
		elements.length ? elements[0]['type'] : undefined
	);
